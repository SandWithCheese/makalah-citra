...
\subsection{Dataset}
Dataset yang digunakan dalam penelitian ini merupakan data seismik riil yang diperoleh dari perusahaan minyak dan gas bumi (\textit{oil and gas}) yang beroperasi di kawasan Asia Tenggara. Dikarenakan sifat kerahasiaan (\textit{confidential}) data industri, detail lokasi dan atribut geofisika spesifik tidak dapat dipublikasikan secara terbuka. Volume dataset yang tersedia untuk penelitian ini relatif terbatas, terdiri dari sekitar sepuluh berkas data penampang seismik yang merepresentasikan struktur bawah permukaan. Terbatasnya jumlah data asli ini menjadi tantangan utama yang diatasi melalui strategi \textit{preprocessing} dan augmentasi data yang sistematis.
...
Potongan kode berikut memperlihatkan implementasi logika iterasi \textit{tiling}:
\begin{figure}[htbp]
\begin{small}
\begin{verbatim}
...
Kode implementasi utama untuk inisiasi pelatihan adalah sebagai berikut:
\begin{figure}[htbp]
\begin{small}
\begin{verbatim}
